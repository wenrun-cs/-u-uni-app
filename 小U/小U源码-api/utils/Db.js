...
class Db {
     ...
     * @resolve true代表插入成功, 字符串代表插入失败
     */
    insertAll(req, tableName, obj) {
        return new Promise((resolve, reject) => {
            let sqlStr = "";
            if (obj !== undefined && typeof obj[0] === "object" && Object.keys(obj[0]).length > 0) {
                // 把对象的key:value拼接成 ( field1, field2,...fieldN )
                //    VALUES
                ...
                for(var i=0;i<obj.length;i++){
                    for (let prop in obj[i]) {
                        valuestr += (typeof obj[i][prop] === 'number' ? obj[i][prop] : "'" + obj[i][prop] + "'") + " , ";
                    }
